...
def clean_numeric_column(column_data: pd.Series, outlier_action: str, missing_action: str) -> pd.Series:
    ...
    Step 1: Outlier Handling – based on the specified outlier_action.
      - "remove_outliers_fill_NA": cells outside the 5th–95th percentiles are masked and then set to the literal string "NA".
      - "remove_outliers_set_missing": cells outside the thresholds are masked (left as np.nan).
      - "remove_outliers_impute_mean/median/mode": outliers are replaced with the corresponding statistic.
    
    Step 2: Missing Value Handling – fills only cells that were originally missing.
      - "fill_NA": fills with the literal string "NA" (column is converted to object).
      - "impute_mean"/"impute_median": fills with computed statistic.
      - "custom:xxx": fills with a custom value.
      - "drop_rows": drops rows with originally missing cells.
      - "none": leaves originally missing cells unchanged.
    """
...
class DataRemarkAgent(BaseAgent):
    ...
    def get_confirmed_decision(self, prompt: str, valid_options: List[str]) -> str:
        while True:
            ...
            if decision not in valid_options:
                ...
                continue
            confirm = input(f"You entered '{decision}'. Press 'Y' to confirm or 'B' to re-enter: ").strip().lower()
            if confirm in ['y', 'yes', '']:
                return decision
            # if 'b', loop again
    ...
    def run(self, analysis: ColumnAnalysis) -> Dict:
        while True:
            ...
            if analysis.null_count > 0:
                ...
                if is_numeric:
                    mapping_missing = {
                        ...
                        "3": "impute_median",
                        "4": "drop_rows",
                        "5": "custom",  # will ask for custom value
                        "6": "none"
                    }
                else:
                    mapping_missing = {
            ...
            # Outlier handling decision (only for numeric columns with outlier issues)
            ...
            final_confirm = input("Press 'Y' to confirm these decisions or 'B' to restart this column's processing: ").strip().lower()
            if final_confirm in ['y', 'yes', '']:
                logger.info(f"User decisions for column '{analysis.column_name}': {decisions}")
                return decisions
            else:
                print("Restarting decision process for this column...")
                continue
    # End of run()
    ...
    def run(self, input_file: str, output_file: str, columns: Optional[List[str]] = None) -> pd.DataFrame:
        try:
            ...
            for col_name in columns_to_clean:
                while True:  # Outer loop for entire column processing
                    print(f"\n{'='*50}\nProcessing column: {col_name}\n{'='*50}")
                    # Data type conversion with confirmation
                    while True:
                    ...
                    final_confirm = input(f"Press 'Y' to confirm these decisions for column '{col_name}', or 'B' to restart this column's processing: ").strip().lower()
                    if final_confirm in ['y', 'yes', '']:
                        break  # Confirmed, exit outer loop for this column
                    else:
                        print("Restarting entire processing for this column...")
                        continue
